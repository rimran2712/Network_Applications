...
'''
this program will Configure IP addesses, OSPF and eBGP as base configureation
...
def config_ip_ebgp_vars_j2_template (task, pbar):
    try:
        ...
        print ('*** An error occured during Loading Hosts_Variables ***')
    ...
    try:
        ...
        print ('*** An error occured during IP Configuration on Interfaces ***')
    
    try:
        # eBGP/iBGP required TCP/IP reachability to neighbors so we need to configure any IGP
        # OSPF configurations using j2 template for iBGP peer reachability
        config_ospf_j2_template(task)
    except:
        print ('*** An error occured during OSPF Configuration ***')
    try:
        # iBGP configuration using j2 template
        config_eBGP_j2_template(task)
    except:
        print ('*** An error occured during eBGP Configuration ***')
