...
def config_ip_ibgp_vars_j2_template (task, pbar):
    try:
    ...
    try:
        ...
        print ('*** An error occured during iBGP Configuration ***')
...
with tqdm (total=len(nr.inventory.hosts), desc = 'Configuring Devices', colour='green') as pbar:
    results = nr.run (task=config_ip_ibgp_vars_j2_template, pbar=pbar)
